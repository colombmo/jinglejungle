...
def setup_fuzzy_engine():
    ...
    emotions_universe = np.arange(0, 1, 0.1)
    ...
    db_level[db_labels[2]] = fuzz.trimf(db_universe, [50, 70, 90])
    db_level_dict[db_labels[2]] = fuzz.trimf(db_universe, [50, 70, 90])
    ...
    db_level_dict[db_labels[4]] = fuzz.trimf(db_universe, [90, 115, 115])
    ...
    anger['low'] = fuzz.trimf(emotions_universe, [0, 0, 0.2])
    anger['medium'] = fuzz.trimf(emotions_universe, [0.1, 0.25, 0.4])
    anger['high'] = fuzz.trimf(emotions_universe, [0.3, 1, 1])
    sadness['low'] = fuzz.trimf(emotions_universe, [0, 0, 0.2])
    sadness['medium'] = fuzz.trimf(emotions_universe, [0.1, 0.25, 0.4])
    sadness['high'] = fuzz.trimf(emotions_universe, [0.3, 1, 1])
    fear['low'] = fuzz.trimf(emotions_universe, [0, 0, 0.2])
    fear['medium'] = fuzz.trimf(emotions_universe, [0.1, 0.25, 0.4])
    fear['high'] = fuzz.trimf(emotions_universe, [0.3, 1, 1])
    ...
    rule17 = ctrl.Rule(anger['high'], perceived_db_level['loud'])
    rule18 = ctrl.Rule(sadness['high'], perceived_db_level['loud'])
    rule19 = ctrl.Rule(fear['high'], perceived_db_level['loud'])
    #rule20 = ctrl.Rule(surprise['high'], perceived_db_level['normal'])
    #rule21 = ctrl.Rule(joy['high'], perceived_db_level['normal'])

    rule22 = ctrl.Rule(anger['medium'], perceived_db_level['normal'])
    rule23 = ctrl.Rule(sadness['medium'], perceived_db_level['normal'])
    rule24 = ctrl.Rule(fear['medium'], perceived_db_level['normal'])
    #rule25 = ctrl.Rule(surprise['medium'], perceived_db_level['normal'])
    #rule26 = ctrl.Rule(joy['medium'], perceived_db_level['normal'])

    rule27 = ctrl.Rule(anger['low'], perceived_db_level['quiet'])
    rule28 = ctrl.Rule(sadness['low'], perceived_db_level['quiet'])
    rule29 = ctrl.Rule(fear['low'], perceived_db_level['quiet'])
    #rule30 = ctrl.Rule(surprise['low'], perceived_db_level['normal'])
    #rule31 = ctrl.Rule(joy['low'], perceived_db_level['normal'])
    ...
    ctrlSystem = ctrl.ControlSystem([rule1, rule2, rule3, rule4, rule5, rule6, rule7, rule8, rule9,
                                     ...
                                     rule18, rule19, rule22, rule23, rule24, rule27, rule28, rule29])
...
#estimator.input['surprise'] = emotions['surprise']
#   estimator.input['joy'] = emotions['joy']
...
def fuzzify_sound_word(word, distance):
    ...
    if distance == 'close':
        ...
        rule7 = ctrl.Rule(frequency['low'], estimate_freq['high'])
        rule8 = ctrl.Rule(frequency['normal'], estimate_freq['high'])
        ...
        rule8 = ctrl.Rule(frequency['normal'], estimate_freq['low'])
        rule9 = ctrl.Rule(frequency['high'], estimate_freq['low'])
    else:
        ...
        rule7 = ctrl.Rule(frequency['low'], estimate_freq['normal'])
        rule8 = ctrl.Rule(frequency['normal'], estimate_freq['normal'])
        rule9 = ctrl.Rule(frequency['high'], estimate_freq['normal'])
...
if __name__ == '__main__':
    ...
    setup_fuzzy_engine()
    emotions = {'anger': 0.3978378534317017, 'sadness': 0.9422713100910187, 'fear': 0.94237992689013481,
                'surprise': 0.024785879999399185, 'joy': 0.020769251510500908}
    print(predict_sound_level('air', 'close', emotions))
